...
class Interpreter:
    ...
    def evaluate_Print(self, node):
        if isinstance(node.expr, ListNode):
            # Handle multiple arguments in print
            values = [self.evaluate(expr) for expr in node.expr.elements]
            self._print(*values)
            return values
        else:
            # Handle single argument print
            value = self.evaluate(node.expr)
            self._print(value)
            return value
    ...
    def evaluate_FunctionDef(self, node):
        # Store the function definition in the functions dictionary
    ...
    def evaluate_StringMethod(self, node):
        string_obj = self.evaluate(node.expr)
